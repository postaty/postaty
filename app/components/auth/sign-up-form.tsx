  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      ...
      <SignUp.Root>
        <SignUp.Step name="start" className="space-y-4 w-full">
          ...
          <div className="relative py-2">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-background px-2 text-muted-foreground">أو باستخدام البريد</span>
            </div>
          </div>

          <Clerk.Field name="emailAddress" className="flex flex-col gap-1.5">
            <Clerk.Label className="text-sm font-medium text-foreground">البريد الإلكتروني</Clerk.Label>
            <Clerk.Input 
              type="email" 
          ...
          <Clerk.Field name="password" className="flex flex-col gap-1.5">
            <Clerk.Label className="text-sm font-medium text-foreground">كلمة المرور</Clerk.Label>
            <Clerk.Input 
              type="password" 
              className="px-4 py-2.5 bg-surface-1 border border-card-border rounded-xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200"
              placeholder="••••••••"
          ...
          <SignUp.Action submit asChild>
            <button className="w-full py-3 bg-gradient-to-r from-primary to-primary-hover text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/25 hover:shadow-primary/40 hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2">
              <Clerk.Loading>
                {(isLoading) => isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "إنشاء حساب"}
              </Clerk.Loading>
            </button>
          </SignUp.Action>
        ...
        {/* Verification Step - often needed for sign up */}
        <SignUp.Step name="verifications" className="space-y-4 w-full">
            <div className="text-center mb-6">
                <h2 className="text-xl font-bold mb-2">تأكيد البريد الإلكتروني</h2>
                <p className="text-sm text-muted-foreground">
                    أرسلنا رمز التحقق إلى بريدك الإلكتروني. الرجاء إدخاله للمتابعة.
                </p>
            </div>

            <SignUp.Strategy name="email_code">
                <Clerk.Field name="code" className="flex flex-col gap-1.5">
                    <Clerk.Label className="text-sm font-medium text-foreground sr-only">رمز التحقق</Clerk.Label>
                    <Clerk.Input 
                        type="text" 
                        className="px-4 py-3 bg-surface-1 border border-card-border rounded-xl text-foreground text-center text-lg tracking-widest placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200"
                        placeholder="123456"
                    />
                    <Clerk.FieldError className="text-xs text-danger mt-1" />
                </Clerk.Field>
                ...
                <SignUp.Action submit asChild>
                    <button className="w-full mt-4 py-3 bg-gradient-to-r from-primary to-primary-hover text-primary-foreground font-bold rounded-xl shadow-lg shadow-primary/25 hover:shadow-primary/40 hover:-translate-y-0.5 transition-all duration-200 flex items-center justify-center gap-2">
                        <Clerk.Loading>
                            {(isLoading) => isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "تأكيد الحساب"}
                        </Clerk.Loading>
                    </button>
                </SignUp.Action>
            </SignUp.Strategy>
